...
//    }
    ...
    public void executeOneStep() throws MyException {
        ...
        repo.setProgramList(removeCompletedProgram(repo.getProgramStates()));
        List<ProgramState> programStates = repo.getProgramStates();
        if(programStates.size() > 0)
        {
            try {
                oneStepForAllProgram(repo.getProgramStates());
            } catch (InterruptedException e) {
                System.out.println("CTRL: "+e.getMessage());
            }
            repo.setProgramList(removeCompletedProgram(repo.getProgramStates()));
        ...
        List<ProgramState> newProgList = executor.invokeAll(callList).stream()
                .map(futureObj -> {
                    try { return futureObj.get(); }
                        ...
                        System.out.println("OneStepAll: "+ignored.getMessage());
                    }
                    return null;
                })
