I chose the Mamdani method with the Centroid Area calculation of the z-point. The rule table implies that between the conditions there's always an AND operation so when evaluating the rules i used the minimums of the fuzzified values. In the end, if the evaluated rules all have a single label then it is the final label, otherwise I calculate the COA from the result fuzzy set and then use the membership function of the output fuzzy set to get the label with the maximum membership.
