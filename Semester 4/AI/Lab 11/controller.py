...
MAX_DEPTH = 7
TERMINALS = ['-165', '-150', '-135', '-120', '-105', '-90', '-75', '-60', '-45', '-30', '-15', '0', '15', '30', '45', '60', '75', '90', '105', '120', '135', '150', '165']
FUNCTIONS = ['+', '-', '*', 'sin', 'cos']
...
def geneticSearch(data, labels, populationSize=1000, replacePerGenerationPercentage=0.2, tournamentPercentage=0.05, mutationChance=0.1, epsilon=0.65):
    ...
    while alphaIndividual is None or alphaIndividual.getAccuracy() < epsilon:
        ...
        try:
            probabilityDistribution = [max(individual.getFitness() for individual in individuals) - individual.getFitness() for individual in individuals]
    ...
    fileWriter.write("fitness: " + alphaIndividual.getFitness() + "accuracy: " + alphaIndividual.getAccuracy())